 ...
 public:
  ...
  privilege_t get_access() const
  {
    ...
    if (num_fields() <= 18)
    {
      ...
      if (access & FILE_ACL)
        access|= REPL_CLIENT_ACL | REPL_SLAVE_ACL | BINLOG_ADMIN_ACL ;
    ...
    if (access & SUPER_ACL)
      access|= GLOBAL_SUPER_ADDED_SINCE_USER_TABLE_ACL;
...
static bool ignore_max_password_errors(const ACL_USER *acl_user)
{
 ...
 return (acl_user->access & IGNORE_MAX_PASSWORD_ERRORS_ACL)
...
bool acl_authenticate(THD *thd, uint com_change_user_pkt_len)
{
  ...
  */
  ...
  if (command == COM_CONNECT &&
      !(thd->main_security_ctx.master_access & IGNORE_MAX_CONNECTIONS_ACL))
  {
