...
static bool deny_updates_if_read_only_option(THD *thd, TABLE_LIST *all_tables)
{
  ...
  if ((thd->security_ctx->master_access & IGNORE_READ_ONLY_ACL) != NO_ACL)
    DBUG_RETURN(FALSE);
...
static my_bool wsrep_read_only_option(THD *thd, TABLE_LIST *all_tables)
{
  ...
  privilege_t flag_saved= thd->security_ctx->master_access & IGNORE_READ_ONLY_ACL;

  opt_readonly = 0;
  thd->security_ctx->master_access &= ~IGNORE_READ_ONLY_ACL;
  ...
  */
  ...
  case COM_BINLOG_DUMP:
    {
      ...
      if (check_global_access(thd, COM_BINLOG_DUMP_ACL))
	break;
  ...
  case COM_PROCESS_INFO:
    ...
    if (!thd->security_ctx->priv_user[0] &&
        check_global_access(thd, COM_PROCESS_INFO_ACL))
      break;
    general_log_print(thd, command, NullS);
    mysqld_list_processes(thd,
                     thd->security_ctx->master_access & COM_PROCESS_INFO_ACL ?
  ...
  case COM_DEBUG:
    status_var_increment(thd->status_var.com_other);
    if (check_global_access(thd, DEBUG_ACL))
      break;					/* purecov: inspected */
    mysql_print_status();
    general_log_print(thd, command, NullS);
    my_eof(thd);
  ...
  else
  {
    ...
    */
    ...
    if (!curuserhost && !currole &&
        check_global_access(thd, SET_USER_ACL, false))
      DBUG_RETURN(TRUE);
  }
...
mysql_execute_command(THD *thd)
{
  ...
  */
  ...
  {
    if (!thd->security_ctx->priv_user[0] &&
        check_global_access(thd, STMT_SHOW_EXPLAIN_ACL))
      break;
...
# endif
  ...
  case SQLCOM_PURGE:
  {
    if (check_global_access(thd, STMT_PURGE_BINLOG_ACL))
      goto error;
  ...
  {
    if (check_global_access(thd, STMT_SHOW_SLAVE_HOSTS_ACL))
      goto error;
  ...
  {
    WSREP_SYNC_WAIT(thd, WSREP_SYNC_WAIT_BEFORE_SHOW);
    if (check_global_access(thd, STMT_SHOW_RELAYLOG_EVENTS_ACL))
      goto error;
  ...
  {
    WSREP_SYNC_WAIT(thd, WSREP_SYNC_WAIT_BEFORE_SHOW);
    if (check_global_access(thd, STMT_SHOW_BINLOG_EVENTS_ACL))
      goto error;
    res = mysql_show_binlog_events(thd);
    break;
  }
  ...
  case SQLCOM_CHANGE_MASTER:
  {
    ...
    if (check_global_access(thd, STMT_CHANGE_MASTER_ACL))
      goto error;
    /*
  ...
  case SQLCOM_SHOW_BINLOG_STAT:
  {
    ...
    if (check_global_access(thd, STMT_SHOW_BINLOG_STATUS_ACL))
      goto error;
  ...
  case SQLCOM_SHOW_ENGINE_STATUS:
    {
      if (check_global_access(thd, STMT_SHOW_ENGINE_STATUS_ACL))
        goto error;
  ...
  case SQLCOM_SHOW_ENGINE_MUTEX:
    {
      if (check_global_access(thd, STMT_SHOW_ENGINE_MUTEX_ACL))
        goto error;
  ...
  */
...
#else
    {
      if (check_global_access(thd, STMT_SHOW_BINARY_LOGS_ACL))
	goto error;
      WSREP_SYNC_WAIT(thd, WSREP_SYNC_WAIT_BEFORE_SHOW);
  ...
  case SQLCOM_SHOW_PROCESSLIST:
    if (!thd->security_ctx->priv_user[0] &&
        check_global_access(thd, STMT_SHOW_PROCESSLIST_ACL))
      break;
    mysqld_list_processes(thd,
                (thd->security_ctx->master_access & STMT_SHOW_PROCESSLIST_ACL ?
                 NullS :
  ...
  case SQLCOM_CREATE_SERVER:
  {
    ...
    if (check_global_access(thd, STMT_CREATE_SERVER_ACL))
      break;
  ...
  case SQLCOM_ALTER_SERVER:
  {
    ...
    if (check_global_access(thd, STMT_ALTER_SERVER_ACL))
      break;
  ...
  case SQLCOM_DROP_SERVER:
  {
    ...
    if (check_global_access(thd, STMT_DROP_SERVER_ACL))
      break;
...
#ifdef WITH_WSREP
    if (((thd->security_ctx->master_access & KILL_OTHER_USER_PROCESS_ACL) ||
